...
public class Controlador {
	...
	// Agregado por grupo
	public List<PedidoView> getPedidosByCliente(String cuit) throws ClienteException {
		Cliente cliente = ClienteDAO.getInstancia().findClienteByCuit(cuit);
		List<Pedido> pedidos = PedidoDAO.getInstancia().findPedidoByCliente(cliente);
		return pedidos.stream()
				.map(pedido -> pedido.toView())
				.collect(Collectors.toList());
	}
	
	// Agregado por grupo
	public ProductoView getProductoById(int numero) throws ProductoException{
		return ProductoDAO.getInstancia().findProductoByIdentificador(numero).toView();
	}
	...
	public List<PedidoView> getPedidos() {
		List<Pedido> pedidos = PedidoDAO.getInstancia().findAll();
		return pedidos.stream()
				.map(pedido -> pedido.toView())
				.collect(Collectors.toList());
	}
